...
try:
    ...
    class Optimizer:
        ...
        def _compute_probs_and_costs(self) -> dict[str, float]:
            review_log_df = pd.DataFrame(vars(review_log) for review_log in self.review_logs)
            ...
            first_reviews_df = review_log_df.loc[~review_log_df["card_id"].duplicated(keep="first")].reset_index(
                drop=True
            )

            first_again_reviews_df = first_reviews_df.loc[first_reviews_df["rating"] == Rating.Again]
            first_hard_reviews_df = first_reviews_df.loc[first_reviews_df["rating"] == Rating.Hard]
            first_good_reviews_df = first_reviews_df.loc[first_reviews_df["rating"] == Rating.Good]
            first_easy_reviews_df = first_reviews_df.loc[first_reviews_df["rating"] == Rating.Easy]
            ...
            num_first_review = num_first_again + num_first_hard + num_first_good + num_first_easy
            ...
            first_again_review_durations = list(first_again_reviews_df["review_duration"])
            ...
            avg_first_again_review_duration = mean(first_again_review_durations) if first_again_review_durations else 0
            avg_first_hard_review_duration = mean(first_hard_review_durations) if first_hard_review_durations else 0
            avg_first_good_review_duration = mean(first_good_review_durations) if first_good_review_durations else 0
            avg_first_easy_review_duration = mean(first_easy_review_durations) if first_easy_review_durations else 0

            probs_and_costs_dict["avg_first_again_review_duration"] = avg_first_again_review_duration
            probs_and_costs_dict["avg_first_hard_review_duration"] = avg_first_hard_review_duration
            probs_and_costs_dict["avg_first_good_review_duration"] = avg_first_good_review_duration
            probs_and_costs_dict["avg_first_easy_review_duration"] = avg_first_easy_review_duration

            # compute the probabilities and costs of non-first ratings
            non_first_reviews_df = review_log_df.loc[review_log_df["card_id"].duplicated(keep="first")].reset_index(
                drop=True
            )

            again_reviews_df = non_first_reviews_df.loc[non_first_reviews_df["rating"] == Rating.Again]
            hard_reviews_df = non_first_reviews_df.loc[non_first_reviews_df["rating"] == Rating.Hard]
            good_reviews_df = non_first_reviews_df.loc[non_first_reviews_df["rating"] == Rating.Good]
            easy_reviews_df = non_first_reviews_df.loc[non_first_reviews_df["rating"] == Rating.Easy]
            ...
            avg_again_review_duration = mean(again_review_durations) if again_review_durations else 0
            avg_hard_review_duration = mean(hard_review_durations) if hard_review_durations else 0
            avg_good_review_duration = mean(good_review_durations) if good_review_durations else 0
            avg_easy_review_duration = mean(easy_review_durations) if easy_review_durations else 0

            probs_and_costs_dict["avg_again_review_duration"] = avg_again_review_duration
            ...
            *,
            ...
            for i in range(num_cards_simulate):
                ...
                while curr_date < end_date:
                    ...
                    card, _ = scheduler.review_card(card=card, rating=rating, review_datetime=curr_date)
                    curr_date = card.due
    ...
    class Optimizer:
        def __init__(self, *args, **kwargs) -> None:
            raise ImportError('Optimizer is not installed.\nInstall it with: pip install "fsrs[optimizer]"')
